...
class TradeSimulator:
    def __init__(self, initial_balance=1000, stop_loss_pct=0.02, target_pct=0.05):
        """
        Initializes the trade simulator with initial balance, stop loss, and target percentage.
        """
    ...
    def simulate_trades(self, df):
        """
        Simulate trades based on given dataframe of historical data
        """
        for index, row in df.iterrows():
            price = row["close"]
            date = index.date()

            # Example ENTRY condition: (SMA50 > SMA200) and RSI < 30
            ...
            # Example EXIT condition: hit stop-loss or target price
        ...
        # Update if there's an open trade
        if self.current_position:
